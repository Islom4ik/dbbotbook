...
bot.command('books', async (ctx) => {
    try {
        if (ctx.session?.booksRequest> (new Date().valueOf() - 5 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты");
...
bot.action('b1', async (ctx) => {
    try {
        if (ctx.session?.b1Request> (new Date().valueOf() - 2 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты");
        ctx.session.b1Request = new Date().valueOf();
        /* main */
        if (ctx.session?.b2Request> (new Date().valueOf() - 2 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты");
        ctx.session.b2Request = new Date().valueOf();

        if (ctx.session?.b3Request> (new Date().valueOf() - 2 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты");
        ctx.session.b3Request = new Date().valueOf();
        
        if (ctx.session?.b4Request> (new Date().valueOf() - 2 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты");
        ctx.session.b4Request = new Date().valueOf();

        if (ctx.session?.b5Request> (new Date().valueOf() - 2 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты");
        ctx.session.b5Request = new Date().valueOf();

        if (ctx.session?.b6Request> (new Date().valueOf() - 2 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты");
        ctx.session.b6Request = new Date().valueOf();

        if (ctx.session?.b7Request> (new Date().valueOf() - 2 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты");
        ctx.session.b7Request = new Date().valueOf();

        if (ctx.session?.b8Request> (new Date().valueOf() - 2 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты");
        ctx.session.b8Request = new Date().valueOf();

        if (ctx.session?.b9Request> (new Date().valueOf() - 2 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты");
        ctx.session.b9Request = new Date().valueOf();

        if (ctx.session?.b10Request> (new Date().valueOf() - 2 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты");
        ctx.session.b10Request = new Date().valueOf();

        if (ctx.session?.b11Request> (new Date().valueOf() - 2 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты ");
        ctx.session.b11Request = new Date().valueOf();

        if (ctx.session?.b12Request> (new Date().valueOf() - 2 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты ");
        ctx.session.b12Request = new Date().valueOf();

        if (ctx.session?.b13Request> (new Date().valueOf() - 2 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты ");
        ctx.session.b13Request = new Date().valueOf();

        if (ctx.session?.b14Request> (new Date().valueOf() - 2 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты ");
        ctx.session.b14Request = new Date().valueOf();

        if (ctx.session?.b15Request> (new Date().valueOf() - 2 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты ");
        ctx.session.b15Request = new Date().valueOf();

        if (ctx.session?.b16Request> (new Date().valueOf() - 2 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты ");
        ctx.session.b16Request = new Date().valueOf();

        if (ctx.session?.b17Request> (new Date().valueOf() - 2 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты ");
        ctx.session.b17Request = new Date().valueOf();

        if (ctx.session?.b18Request> (new Date().valueOf() - 2 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты ");
        ctx.session.b18Request = new Date().valueOf();

        if (ctx.session?.b19Request> (new Date().valueOf() - 2 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты ");
        ctx.session.b19Request = new Date().valueOf();

        if (ctx.session?.b20Request> (new Date().valueOf() - 2 * 60 * 1000)) return await ctx.reply(" Вы можете повторно запросить документ через 2 минуты ");
